...
st.set_page_config(page_title="Adaptive CRAG Agent",page_icon="🧠",layout="centered")
st.html("""<style>.stAppDeployButton {display:none;}#MainMenu {visibility: hidden;}</style>""")
...
with st.sidebar:
    ...
    st.markdown(
        """
        **Adaptive CRAG Agent** Retrieval-Augmented Generation with:
        - Self-Correction
        - Query Rewriting
        - Web Search Fallback
        """
        ...
        if documents:
          with st.expander("📚 Referenced Sources"):
            for i, doc in enumerate(documents):
             source = doc.metadata.get("source") or doc.metadata.get("url") or "Database"
